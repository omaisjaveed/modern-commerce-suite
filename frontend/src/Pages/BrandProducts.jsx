...
function BrandProducts() {
  ...
  useEffect(() => {
    const fetchBrandAndProducts = async () => {
      try {
        // First get all brands to find the ID for this slug
        const brandRes = await API.get('/brands');
        const foundBrand = brandRes.data.find(b => b.slug === slug);
        
        if (foundBrand) {
          setBrand(foundBrand);
          const res = await API.get(`/products?brand_id=${foundBrand.id}`);
          setProducts(res.data.products || res.data || []);
        } else {
          // Fallback to all products if brand not found
  ...
  const brandTitle = brand ? brand.name : 'Brand';

  return (
    <>
      <Helmet>
        <title>{brandTitle} | Jewelery For All Occasions</title>
        <meta name="description" content={`Browse ${brandTitle} products at Jewelery For All Occasions.`} />
      </Helmet>
      <InnerBanner InnerBannerText={brandTitle} />
      <section className="shop-sec category-products-sec">
        <div className="container">
          <div className="products-header category-products-header">
            <p>Showing {products.length} products in {brandTitle}</p>
          </div>
